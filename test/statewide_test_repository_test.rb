...
class StateWideTestTest < Minitest::Test
  ...
  def test_shit
    ...
    assert_equal expected_one, statewide_object.grade_three_proficient
    ...
    assert_equal expected_two, statewide_object.grade_eight_proficient
    ...
    actual_one = statewide_object.proficient_by_grade(3)
    expected_three = {2008=>{:math=>0.857, :reading=>0.866, :writing=>0.671},
                   2009=>{:math=>0.824, :reading=>0.862, :writing=>0.706},
                   2010=>{:math=>0.849, :reading=>0.864, :writing=>0.662},
                   2011=>{:math=>0.819, :reading=>0.867, :writing=>0.678},
                   2012=>{:reading=>0.87, :math=>0.83, :writing=>0.655},
                   2013=>{:math=>0.855, :reading=>0.859, :writing=>0.668},
                   2014=>{:math=>0.834, :reading=>0.831, :writing=>0.639}}
    assert_equal expected_three, actual_one
    #need a test for when grad isn't there
    actual_one = statewide_object.proficient_by_grade(13)
    assert_raises (UnknownDataError) {actual_one}

    # " when input is incorrect

    actual_two = statewide_object.proficient_by_race_or_ethnicity(:white)
    expected_four = {2011=>{:math=>0.706, :reading=>0.851, :writing=>0.74},
                     2012=>{:math=>0.713, :reading=>0.861, :writing=>0.726},
                     2013=>{:math=>0.72,  :reading=>0.86,  :writing=>0.74},
                     2014=>{:math=>0.723, :reading=>0.008, :writing=>0.734}}
    assert_equal expected_four, actual_two
    #" for when race isn't there
    # " when input is incorrect

    actual_three = statewide_object.proficient_for_subject_by_race_in_year(:math, :asian, 2011)
    ...
    actual_four = statewide_object.proficient_for_subject_by_grade_in_year(:math, 3, 2008)
